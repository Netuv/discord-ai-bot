# 🏗️ Hybrid Cloudflare Workers + Vercel — Turbo Layer Plan
...
>
> 🚨 **UPDATE 21 Juni 2026:** Provider final = **Vercel** (Hobby, gratis, tanpa credit card).
> Kode server pake Express + serverless function (`api/index.js`). Provider-agnostic.
...
| [**2. Arsitektur Hybrid**](#-2-arsitektur-hybrid) | Gambaran besar — hubungan Worker ⇄ Vercel |
...
║   │                                                       │  ║
║   │   TUGAS: Jembatan ke Turbo Layer                           │  ║
║   │   - tryRenderHeavy() → call Turbo Layer API                │  ║
...
Worker ──panggil─→ Render
  ...
  │  Kalau Turbo Layer mati:│  Jalanin logic Worker seperti biasa
```
...
async function callRender<T = any>(
  ...
  // Kalau Turbo Layer gak dikonfigurasi → skip (silent)
  if (!RENDER_URL) {
...
if (cmdName === "ask") {
  ...
  ctx.waitUntil((async () => {
    try {
      ...
      console.log('⚠️ Turbo Layer unavailable — Worker fallback untuk /ask');
...
| **Tidak ada perf degradation** | Kalau Turbo Layer lambat → Worker gak nunggu, pake punya sendiri |
...
Skenario ───────────────┼─ Render hidup, lambat → Worker pake punya sendiri ✅
                        ...
                        │
                        ├─ Turbo Layer mati → Worker jalan seperti biasa ✅
                        │   (Gak ada perubahan)
                        │
                        └─ Turbo Layer gak dikonfigurasi → Worker jalan seperti biasa ✅
...
```bash
npx wrangler tail
# Cari: "🚀 Turbo Layer success" atau "⚠️ Turbo Layer unavailable"
```

Turbo Layer logs:
```
Dashboard Vercel → Service → Logs
...
```bash
# Hapus TURBO_SERVICE_URL → render-helper otomatis return null
...
### Opsi 2: Clean (kalau mau hapus semua kode Turbo Layer)
...
3. ❌ Jangan ubah format artikel — prompt di Turbo Layer harus SAMA dengan di `article-writer.ts`
...
> **"Kalau Turbo Layer mati, gak ada response, atau kehapus — bot tetap berfungsi PERSIS seperti sebelum Turbo Layer ada. Tidak ada perubahan perilaku, tidak ada error, tidak ada data loss."**
